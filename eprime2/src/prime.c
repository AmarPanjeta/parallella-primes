...
int main(int argc, char *argv[])
{
	...
	/*uint64_t*/ long long int x_value = DEFAULT_X;

	if(argc > 1)
	{
		x_value = strtoull(argv[1], NULL, 10);
	}
	printf("unijeti broj je: %lld\n",x_value);
	...
	for(row=0;row<platform.rows;row++)
	{
		...
		{
			e_write(&dev, row, col, 0x7020, &x_value, sizeof(long long int));
		}
	}
	...
	while(1)
	{
		...
		for(row=0;row<platform.rows;row++)
		{
			for(col=0;col<platform.cols;col++)
			{
				...
				if(e_read(&dev, row, col, 0x7010/*0x7008*/, &current, sizeof(int64_t)) != sizeof(int64_t))
	...
	printf("Prost broj %lld, a done je %d\n",sum,done);
	e_finalize();
